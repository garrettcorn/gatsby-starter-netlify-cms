	...
	graphql.ObjectConfig{
		...
		Fields: graphql.Fields{
			...
			*/
			...
			/* Send a message via Telegram Bot on /api/contact/me
			query={msgtele(email:"email",msg:"msg")}
			*/
			"msgtele": &graphql.Field{
				Type:        graphql.String,
				Description: "Send a message to using the /api/contact/me endpoint",
				Args: graphql.FieldConfigArgument{
				...
				Resolve: func(p graphql.ResolveParams) (interface{}, error) {
					...
					if ok {
						...
						if ok {
							apiEndpoint := "https://" + host + "/api/contact/me"
							v := url.Values{}
							v.Set("user_email", email)
							v.Set("user_message", msg)
							v.Set("user_remote_addr", remoteAddr)
							_, err := http.PostForm(apiEndpoint, v)
							if err != nil {
								return "err PostForm", nil
							}
							return "ok", nil
						}
					}
					return "error - please ensure all fields are valid", nil
				},
			},
		},
	})
